...
- **发现**:findings **必须去重**(同 `inspector_name` + 同 `message` 只渲一次)、**按 severity 降序排**(critical → warning → info)、每条**带来源** `inspector_name`。
- **健康态**:无 findings 时渲染 `✅ 未发现异常`,**禁止**渲染空的「发现」段。
- **多 target**:report 含多主机 findings 时**按主机分节**渲染(每节主机名 + 该主机 severity);单主机不分节。
...
- **当** report 含 2 条 `inspector_name` 与 `message` 完全相同的 finding，及一条更低 severity 的 finding
- **那么** 相同的**必须**只渲一次;critical **必须**排在 warning 之前;每条**必须**带 `inspector_name` 来源
