...
> 🚧 **开发中（M0–M9 已落地，未发 PyPI）** —— `src/` 已有可运行代码，`hostlens` CLI 可装可跑：**M0** 脚手架 + doctor、**M1** ExecutionTarget / Inspector / Report + `hostlens inspect`、**M2** 手写 Agent loop（自然语言意图 → 自选 Inspector → markdown 报告）、**M3** Diagnostician + 根因假设 + regression diff、**M4** Scheduler（cron/interval 定时巡检 + Run 留痕 + daemon 优雅停机 + `hostlens schedule`）、**M5** Notifier（Telegram MarkdownV2 + 飞书 Lark HMAC 签名 + `only_if` 路由 + Scheduler↔Notifier 接线 + `hostlens notify`）、**M6**（✅ 域全满）内置 Inspector 库扩充（`builtin/` 下 72 个 inspector，覆盖 Linux/网络/Nginx/MySQL/PostgreSQL/Redis/Docker/K8s 控制面/安全基线/包管理/TLS/JVM·Go 运行时/日志等域）、**M7** MCP Server（stdio + `hostlens mcp serve`，暴露只读巡检三件套给远程 LLM）、**M8** Docker / Kubernetes ExecutionTarget（只读 exec 进容器 / Pod 巡检，容器安全的 28 个 inspector 直接可用）、**M9** 受控修复（`plan → approve → execute → verify → rollback`；**风险分级**：low 自动执行闭环、medium/high 只产 runbook 不代执行 + append-only audit log + `hostlens fix`）。尚未发布 PyPI 包（本地 `pip install -e` 安装）。
>
> 已实现并可直接用的命令：`inspect` / `demo` / `schedule` / `notify` / `doctor` / `reports` / `mcp` / `fix`（Telegram / 飞书 **通知推送**随 M5 落地：`schedule` 的 `notify` 路由会在 Report 持久化后实际派发；**MCP Server** 随 M7 落地：`hostlens mcp serve` stdio 暴露 `list_inspectors` / `list_targets` / `run_inspector`；**受控修复** 随 M9 落地：`hostlens fix` 走风险分级执行）。下面 M10（通道扩展 / PyPI 发布）章节展示的是 **目标 UX 与规划中的命令形态** —— 尚未实现。
>
...
- 🔒 **受控修复（M9 已落地）** — 写操作严格走 `plan → human approve → execute → verify → rollback-ready`，默认只读。**风险分级**：仅 low 风险走自动执行闭环，medium/high 只产人工 runbook 不代执行（AI 不担中高风险责任）；全程 append-only audit log，`hostlens fix <run_id>` 入口、拒绝 root、非交互缺 `--yes` 退出 1
...
- [x] Remediation 工作流（plan → approve → execute → rollback；风险分级：low 自动 / medium·high 出 runbook）
