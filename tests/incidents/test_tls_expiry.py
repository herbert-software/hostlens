"""Incident snapshot: TLS certificate expiry (parameterized).
...
async def test_tls_expiry(llm_cassette: Callable[..., LLMBackend]) -> None:
    backend = llm_cassette("incident_tls_expiry")
    await assert_incident_snapshot("tls_expiry", backend)
