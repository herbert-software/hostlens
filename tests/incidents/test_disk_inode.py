"""Incident snapshot: disk full and inode exhaustion.
...
async def test_disk_inode(llm_cassette: Callable[..., LLMBackend]) -> None:
    backend = llm_cassette("incident_disk_inode")
    await assert_incident_snapshot("disk_inode", backend)
