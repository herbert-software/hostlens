"""Incident snapshot: dependency TCP connectivity (parameterized).
...
async def test_dependency_unreachable(llm_cassette: Callable[..., LLMBackend]) -> None:
    backend = llm_cassette("incident_dependency_unreachable")
    await assert_incident_snapshot("dependency_unreachable", backend)
