"""Incident snapshot: recent error burst (sampling_window).
...
async def test_error_burst(llm_cassette: Callable[..., LLMBackend]) -> None:
    backend = llm_cassette("incident_error_burst")
    await assert_incident_snapshot("error_burst", backend)
